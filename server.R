...
# plot variables
main.dp <- input$type
average <- mean(us.date[,..type])
total <- max(us.date[, ..input$type])
...
function(input, output) {
  ...
  output$plot <- renderPlot( {
    ...
    g <- ggplot(data = us.date) +
  geom_col(aes(x = date, y = main.dp), fill = "darkorange3") +
  # geom_line(aes(x = date, y = five_day_cases), colour = 'blue', size = 1) +
  geom_hline(aes(yintercept = average), colour = 'grey2', size = 1) +
  scale_x_date(limits = c(start.date, end.date)) +
  labs(title = "Daily Deaths Covid-19 in US by Date — 29 February to current (",end.date,")"),
       subtitle = paste0(paste("Total US ",input$type," :: ", total ), 
                         paste(input$type," per million :: ", (total / 327) ), 
                         paste("Days since 29 February 2020 :: ", d[[1]])
